...
3)
    ...
    docker run -d --name pma_$name --network network-$name -e PMA_HOST=sql_$name -p $port_pma:80 phpmyadmin/phpmyadmin
    ;;
4)
...
5)
    ...
    docker container run -d -p $port:80 --name $name diouxx/glpi:latest
    ;;
6)
    ...
    docker container run -d -p $port:80 --name $name nextcloud
    ;;
7)
    ...
    docker container run -d -p $port:9090 --name $name ubuntu/prometheus:2-24.04_stable
    ;;
8)
    ...
    docker container run -d -p $port:3000 --name $name grafana/grafana
    ;;
9)
...
1)
    ...
    docker container run -d -p $port:80 --name $name nginx
    ;;
2)
    port=$(dialog --inputbox "Entrez le port pour Nginx :" 8 40 2>&1 >/dev/tty)
    name=$(dialog --inputbox "Entrez le nom du conteneur Nginx :" 8 40 2>&1 >/dev/tty)
    repertory=$(dialog --inputbox "Entrez le chemin du répertoire local à monter pour Nginx :" 8 40 2>&1 >/dev/tty)
    docker container run -d -p $port:80 -v $repertory:/usr/share/nginx/html --name $name nginx
    ;;
esac
esac
